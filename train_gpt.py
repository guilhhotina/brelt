...
class Hyperparameters:
    ...
    val_batch_bytes = int(os.environ.get("VAL_BATCH_BYTES", os.environ.get("VAL_BATCH_SIZE", 262_144)))
    ...
    train_batch_bytes = int(os.environ.get("TRAIN_BATCH_BYTES", os.environ.get("TRAIN_BATCH_TOKENS", 262_144)))
    ...
    max_patch_len = int(os.environ.get("MAX_PATCH_LEN", 24))
    max_patches = int(os.environ.get("MAX_PATCHES", 0))
    ...
    global_mlp_mult = float(os.environ.get("GLOBAL_MLP_MULT", os.environ.get("MLP_MULT", 2.0)))
    local_mlp_mult = float(os.environ.get("LOCAL_MLP_MULT", os.environ.get("MLP_MULT", 2.0)))
    ...
    global_xsa_last_n = int(os.environ.get("GLOBAL_XSA_LAST_N", 3))
    global_xsa_start_step = int(os.environ.get("GLOBAL_XSA_START_STEP", 96))
    ...
    recurrence_start_layers = int(os.environ.get("RECURRENCE_START_LAYERS", 9))
    ...
    adaptive_recurrence_min_scale = float(os.environ.get("ADAPTIVE_RECURRENCE_MIN_SCALE", 0.75))
    adaptive_recurrence_target = float(os.environ.get("ADAPTIVE_RECURRENCE_TARGET", 0.75))
    adaptive_recurrence_super_target = float(os.environ.get("ADAPTIVE_RECURRENCE_SUPER_TARGET", 24.0))
    ...
    local_dim = int(os.environ.get("LOCAL_DIM", 144))
    local_encoder_layers = int(os.environ.get("LOCAL_ENCODER_LAYERS", 1))
    ...
    boundary_target_patch_size = float(os.environ.get("BOUNDARY_TARGET_PATCH_SIZE", 8.0))
    boundary_rate_weight = float(os.environ.get("BOUNDARY_RATE_WEIGHT", 0.1))
    boundary_smooth_alpha = float(os.environ.get("BOUNDARY_SMOOTH_ALPHA", 0.25))
    boundary_proxy_weight = float(os.environ.get("BOUNDARY_PROXY_WEIGHT", 0.05))
    boundary_bias_eta = float(os.environ.get("BOUNDARY_BIAS_ETA", 0.5))
    boundary_bias_min = float(os.environ.get("BOUNDARY_BIAS_MIN", -8.0))
    boundary_bias_max = float(os.environ.get("BOUNDARY_BIAS_MAX", 8.0))
    ...
    qat_start_step = int(os.environ.get("QAT_START_STEP", 96))
    qat_bits = int(os.environ.get("QAT_BITS", 8))
    dual_rate_enabled = bool(int(os.environ.get("DUAL_RATE_ENABLED", "1")))
    dual_rate_capacity = float(os.environ.get("DUAL_RATE_CAPACITY", 1.0))
    dual_rate_eta = float(os.environ.get("DUAL_RATE_ETA", 0.01))
